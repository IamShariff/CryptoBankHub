...
import com.cbh.cryptoservice.util.Constants;
...
public class WebclientConfig {
	...
	public WebClient.Builder webClientBuilder() {
		return WebClient.builder().baseUrl(Constants.COIN_CAP_BASE_URL)
	...
	public WebClient webclient() {
		ExchangeStrategies exchangeStrategies = ExchangeStrategies.builder().codecs(this::customCodecs).build();

		return webClientBuilder().exchangeStrategies(exchangeStrategies).build();
	}
	...
	private ExchangeFilterFunction oauthTokenFilter() {
		return ExchangeFilterFunction.ofRequestProcessor(clientRequest -> {
			...
			ClientRequest authorizedRequest = ClientRequest.from(clientRequest)
					.headers(headers -> headers.setBearerAuth(Constants.COIN_CAP_TOKEN)).build();
	...
	private <T> List<T> parseJsonResponse(String jsonResponse, TypeReference<List<T>> responseType) {
		...
		try {
			JsonNode jsonNode = objectMapper().readTree(jsonResponse).get("data");
	...
	/**
	 * Custom ExchangeStrategies for increasing buffer size limit.
	 *
	 * @param configurer ClientCodecConfigurer
	 */
	private void customCodecs(ClientCodecConfigurer configurer) {
		configurer.defaultCodecs().maxInMemorySize(1024 * 1024 * 10); // Set the buffer size limit to 10 MB
	}
}
